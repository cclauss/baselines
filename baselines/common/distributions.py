...
class CategoricalPd(Pd):
    ...
    def neglogp(self, x):
        ...
        # For more stability:
        logits = tf.clip_by_value(self.logits, tf.constant(-5, dtype=tf.float32), tf.constant(5, dtype=tf.float32))
    ...
    def entropy(self):
        a0 = self.logits - tf.reduce_max(self.logits, axis=-1, keep_dims=True)
        # For more stability:
        a0 = tf.clip_by_value(a0, tf.constant(-5, dtype=tf.float32), tf.constant(5, dtype=tf.float32))
        ea0 = tf.exp(a0)
        z0 = tf.reduce_sum(ea0, axis=-1, keep_dims=True)
        p0 = ea0 / z0
    ...
    def sample_temp(self, temperature):
        u = tf.random_uniform(tf.shape(self.logits))
        return tf.argmax(self.logits - tf.log(-tf.log(u)) * temperature, axis=-1)
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
